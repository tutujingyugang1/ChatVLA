...
class MixDataMSELoss(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, outputs, targets, weights=None):
        ...
        if weights is not None:
            ...
            local_denominator = torch.tensor(targets.size(0),device=targets.device)
        sync_tensor = torch.tensor([local_loss.item(),local_denominator.item()], device=targets.device)
        dist.all_reduce(sync_tensor,op=dist.ReduceOp.SUM)
        global_loss=sync_tensor[0]/sync_tensor[1]
        return global_loss
###################
